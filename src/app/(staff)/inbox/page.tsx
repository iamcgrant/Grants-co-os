    ...
    <div className="gc-fade-up">
      <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <h1 className="text-4xl mb-2">Inbox</h1>
          <p className="gc-section-sub mb-0">
            Client messages and internal team chat — visually separated so internal notes never leave the company by accident.
          </p>
        </div>
        <div className="flex gap-2">
          <Link href="/inbox?tab=all" className={`gc-btn ${tab === "all" ? "gc-btn-primary" : "gc-btn-outline"}`}>
            All
          </Link>
          <Link href="/inbox?tab=client" className={`gc-btn ${tab === "client" ? "gc-btn-primary" : "gc-btn-outline"}`}>
            Client
          </Link>
          <Link href="/inbox?tab=team" className={`gc-btn ${tab === "team" ? "gc-btn-primary" : "gc-btn-outline"}`}>
            Team
          </Link>
        </div>
      </div>

      <div className="grid lg:grid-cols-[320px_1fr] gap-5 min-h-[60vh]">
        <aside className="gc-panel overflow-hidden">
            ...
            {conversations.map((conv) => {
              ...
              return (
                <Link
                  ...
                  className={`block px-4 py-3 hover:bg-white/[0.03] ${c === conv.id ? "bg-white/[0.05]" : ""}`}
                >
        ...
        <section className="gc-panel flex flex-col min-h-[60vh]">
          {!active && (
