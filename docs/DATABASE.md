...
`Partner` is a business referral row (Engine A) — never a `Client`. `Client.acquisitionStage` / `acquisitionSource` / `grantsLeadScore` are Engine B fields on the existing master. `PartnerReferral` links partner → client only after conversion. See `docs/ACQUISITION.md`.
