...
function senderIsTrusted({ event, trustedWebContentsId, trustedFileUrl }) {
  ...
  if (/os\.grantandconsultants\.com|gohighlevel|telegram\.org|experian|equifax|disputeprocess|cloudtaxoffice|consumerfinance/i.test(url)) {
    return false;
  }
  return true;
}
...
function validatePayload(op, payload, fsAdapter) {
  ...
  switch (op) {
    ...
    case "open-in-apple-messages": {
      if (op === "subscribe-new" && !payload?.conversationId) {
        return { ok: true, payload: {} };
      }
      if (op === "open-in-apple-messages" && !payload?.conversationId) {
        return { ok: true, payload: {} };
      }
